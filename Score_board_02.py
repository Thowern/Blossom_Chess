...
def score_board(gs):
    ...
    # Definisci le bitboard per i pezzi bianchi e neri
    piece_bitboards = {
    ...
    # Calcola il punteggio per i pezzi e la loro posizione
    for piece, bitboard in piece_bitboards.items():
        num_pieces = np.count_nonzero(bitboard)
        if num_pieces == 0:
            continue
        ...
        if piece_type != 'K':
            score += piece_score[piece_type.lower()] * num_pieces * (1 if color == 'w' else -1)

        # Bonus centrale o tabelle di valutazione
        if phase == 'op' and piece_type in ['p', 'N', 'Q']:
            score += np.dot(central_bonus[color], bitboard) if piece_type != 'Q' else -np.dot(central_bonus[color], bitboard)
        elif phase in ['mg', 'eg']:
            eval_table = eval_tables[f'{color.lower()}{piece_type.lower()}_{phase[:2]}']
            score += np.dot(eval_table, bitboard)
    return score